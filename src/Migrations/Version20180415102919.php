...
class Version20180415102919 extends AbstractMigration
{
    public function up(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__category_entity AS SELECT id, name, hidden FROM category_entity');
        $this->addSql('DROP TABLE category_entity');
        $this->addSql('CREATE TABLE category_entity (id INTEGER NOT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY, hidden BOOLEAN NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO category_entity (id, name, hidden) SELECT id, name, hidden FROM __temp__category_entity');
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__category_entity AS SELECT id, name, hidden FROM category_entity');
        $this->addSql('DROP TABLE category_entity');
        $this->addSql('CREATE TABLE category_entity (id INTEGER NOT NULL, name VARCHAR(255) NOT NULL, hidden BOOLEAN DEFAULT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO category_entity (id, name, hidden) SELECT id, name, hidden FROM __temp__category_entity');
        $this->addSql('DROP TABLE __temp__category_entity');
        $this->addSql('DROP INDEX IDX_C43B1C7AA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__comment_entity AS SELECT id, user_id, content FROM comment_entity');
